...
int Manage_Friend(int recv_account,int send_account,int flag)
{
    if(flag == 1)
    {
        ...
        /*
        current = head_chat;
        new_friend->item.chat.send_user = recv_account;
        new_friend->item.chat.recv_user = send_account;
        strcpy(new_friend -> item.chat.messages,"已添加该用户为好友！");
        new_friend->item.chat.send_can_look = 1;
        new_friend->item.chat.recv_can_look = 1;
        new_friend->item.chat.message_type = 1;
...
/*
            ...
            while(current != NULL)
            {
                printf("account = %d name = %s member_account = %d member_nickname = %s\n",
...
int Add_Group(int send_account,int group)
{
    ...
    else if(flag == 1)
    {
        ...
        current = head_chat;
        printf("------------------------chat_messages--------------------------------------------------------------------------\n");
        while(current != NULL)
        {
            printf("%d %d %s %d %d %d %s\n",current -> item.chat.send_user,current -> item.chat.recv_user,
            current -> item.chat.messages,current -> item.chat.send_can_look,current -> item.chat.recv_can_look,
            current -> item.chat.message_type,current -> item.chat.date);
            current  = current -> next;
        }
        printf("----------------------------------------------------------------------------------------------------------------\n");
        char string[1024] = "\0";
...
int Out_Member(int account,int group_account,int accounted)
{
    ...
    while(current != NULL)
    {
        ...
        {
            flag = 1;
            if(current -> item.member.group_state == 1 || current -> item.member.group_member_account == 2)
            {
                flag = 2;
            }
            break;
